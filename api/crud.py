...
def _apply_patches():
    with sqlite3.connect(DB_PATH) as conn:
        ...
        try:
            ...
            pass
        try:
            cursor.execute("ALTER TABLE patients ADD COLUMN is_active INTEGER DEFAULT 1")
        except sqlite3.OperationalError:
            pass
_apply_patches()
...
def create_patient_by_staff(first_name: str, last_name: str, national_id: str, phone_number: str, gender: str = "male", insurance: str = "none") -> int:
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if row:
            if row[1] == 0:
                cursor.execute(
                    "UPDATE patients SET first_name = ?, last_name = ?, phone_number = ?, gender = ?, insurance = ?, is_active = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (first_name, last_name, phone_number, gender, insurance, row[0])
                )
                conn.commit()
                return row[0]
            else:
        ...
        cursor.execute(
            "INSERT INTO patients (first_name, last_name, national_id, phone_number, gender, insurance, is_active) VALUES (?, ?, ?, ?, ?, ?, 1)",
...
def get_todays_appointments() -> List[Dict[str, Any]]:
    with sqlite3.connect(DB_PATH) as conn:
        ...
        today = date.today().isoformat()
        query = """
            SELECT a.*, p.first_name, p.last_name, p.national_id, p.phone_number, p.gender as patient_gender, s.name as base_service_name
            FROM appointments a
            LEFT JOIN patients p ON a.patient_id = p.id
            LEFT JOIN services s ON a.service_id = s.id
        ...
        for row in cursor.fetchall():
            ...
            gendered_services = ["بادکش", "حجامت عام", "زالودرمانی"]
            if any(s in base_name for s in gendered_services):
                if target_gender == "male": d["service_name"] = f"{base_name} آقایان"
                elif target_gender == "female": d["service_name"] = f"{base_name} بانوان"
                else: d["service_name"] = base_name
            else:
