...
// تابع هوشمند برای تولید شماره صفحات (نقطه‌چین برای اعداد بزرگ)
...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen px-8 pt-12 pb-8 bg-gray-50">
      <h1 className="text-3xl font-bold text-gray-800 mb-8 mt-2">داشبورد خلاصه وضعیت</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          ...
          { title: "کل نوبت‌های فعال", val: stats.active_appointments, icon: CalendarIcon, bgIcon: "bg-green-500", textColor: "text-green-600" },
            ...
            <div className="overflow-x-auto rounded-xl border border-gray-100 mb-6">
              <table className="w-full text-right">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    <th className="p-4 font-medium text-gray-600 text-sm">نام بیمار</th>
                    <th className="p-4 font-medium text-gray-600 text-sm">ساعت</th>
                    <th className="p-4 font-medium text-gray-600 text-sm">شماره تماس</th>
                    <th className="p-4 font-medium text-gray-600 text-sm">خدمت درمانی</th>
                    <th className="p-4 font-medium text-gray-600 text-sm text-center">وضعیت حضور</th>
                  </tr>
                </thead>
                      ...
                      <td className="p-4 flex justify-center">
                        {a.status === 'scheduled' ? (
                          <div className="flex gap-2">
                            <button onClick={() => handleStatusChange(a.id, 'accepted')} className="text-xs bg-green-50 text-green-600 font-bold px-4 py-2 rounded-lg hover:bg-green-100 transition-colors flex items-center gap-1.5"><CheckCircle className="w-4 h-4" /> حضور</button>
                            <button onClick={() => handleStatusChange(a.id, 'no_show')} className="text-xs bg-red-50 text-red-600 font-bold px-4 py-2 rounded-lg hover:bg-red-100 transition-colors flex items-center gap-1.5"><X className="w-4 h-4" /> غایب</button>
                          </div>
                        ) : (
