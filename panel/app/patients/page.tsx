...
export default function PatientsPage() {
  ...
  const handleCloseModal = () => {
    setIsModalOpen(false); setEditMode(false); setEditId(null); setNewFirstName(""); setNewLastName(""); setNewNationalId(""); setNewPhone(""); setNewGender("male"); setSubmitError("");
  };
  ...
  return (
    <div className="min-h-screen px-8 pt-12 pb-8 relative bg-gray-50">
      <header className="mb-8 flex flex-col md:flex-row md:justify-between md:items-center gap-4 mt-2">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto items-center">
          ...
          <button onClick={() => { setEditMode(false); setIsModalOpen(true); }} className="flex h-11 items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 rounded-xl font-medium shadow-sm whitespace-nowrap"><UserPlus className="w-5 h-5" /> ثبت بیمار جدید</button>
          <button onClick={handleExportExcel} className="flex h-11 items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 rounded-xl font-medium shadow-sm whitespace-nowrap"><Download className="w-5 h-5" /> خروجی اکسل</button>
        </div>
      </header>
              ...
              {currentRows.map((patient, index) => (
                <tr key={patient.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-4 text-sm font-medium">
                    <span className={patient.user_id ? "text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg" : "text-gray-600 bg-gray-50 px-3 py-1.5 rounded-lg"}>
                      {patient.user_id ? "ربات بله" : "پنل منشی"}
                    </span>
                  </td>
                  <td className="p-4 flex justify-center gap-3">
                    <button onClick={() => openEditModal(patient)} className="flex items-center gap-1.5 px-4 py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-xl text-xs font-bold transition-colors"><Edit className="w-4 h-4" /> ویرایش</button>
                    <button onClick={() => setDeletePatientData({id: patient.id, name: `${patient.first_name} ${patient.last_name}`})} className="flex items-center gap-1.5 px-4 py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl text-xs font-bold transition-colors"><Trash2 className="w-4 h-4" /> حذف</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {deletePatientData && (
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden transform transition-all border border-gray-100">
            <div className="p-8">
              ...
              <p className="text-center text-gray-600 mb-8 leading-relaxed">
                آیا مطمئن هستید که می‌خواهید پرونده «<span className="font-bold text-gray-900">{deletePatientData.name}</span>» را حذف کنید؟
              </p>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-8">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm font-medium text-amber-800 leading-relaxed">
                    نکته مهم: برای حفظ یکپارچگی سیستم، نوبت‌های قبلی این بیمار در جدول نوبت‌ها حفظ خواهند شد.
                  </p>
                </div>
              </div>
              
              <div className="flex gap-4">
                <button onClick={() => setDeletePatientData(null)} className="flex-1 py-3.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-bold transition-colors">
                  انصراف
                </button>
                <button onClick={confirmDelete} className="flex-1 py-3.5 bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold transition-colors shadow-lg shadow-red-200">
                  بله، حذف شود
                </button>
              </div>
            </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-gray-50">
              <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                {editMode ? <Edit className="w-5 h-5 text-blue-600" /> : <UserPlus className="w-5 h-5 text-blue-600" />} 
                {editMode ? "ویرایش اطلاعات بیمار" : "تشکیل پرونده بیمار جدید"}
              </h3>
            ...
            <form onSubmit={handleSubmitPatient} className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  <input type="text" placeholder="کد ملی ۱۰ رقمی..." className="w-full pl-12 pr-4 h-12 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-right font-mono tracking-widest placeholder:text-gray-400 placeholder:font-sans" dir="rtl" value={newNationalId} onChange={(e) => setNewNationalId(e.target.value.replace(/\D/g, '').slice(0, 10))} required />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">نام:</label><input type="text" value={newFirstName} onChange={e => setNewFirstName(e.target.value.replace(/[0-9۰-۹]/g, ''))} className="w-full px-4 h-12 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" required /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">نام خانوادگی:</label><input type="text" value={newLastName} onChange={e => setNewLastName(e.target.value.replace(/[0-9۰-۹]/g, ''))} className="w-full px-4 h-12 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" required /></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <input type="text" value={newPhone} onChange={e => setNewPhone(e.target.value.replace(/\D/g, '').slice(0, 11))} placeholder="09123456789" dir="ltr" className="w-full pl-10 pr-3 h-12 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-center font-mono tracking-widest" required />
              ...
              <div className="pt-4 flex gap-3">
                <button type="button" onClick={handleCloseModal} className="flex-1 px-4 py-3 bg-white border border-gray-300 text-gray-700 rounded-xl font-medium hover:bg-gray-50 transition-colors">انصراف</button>
                <button type="submit" disabled={submitLoading} className="flex-1 px-4 py-3 bg-blue-600 text-white rounded-xl font-medium hover:bg-blue-700 transition-colors disabled:opacity-70">{submitLoading ? "در حال پردازش..." : (editMode ? "ذخیره تغییرات" : "ذخیره پرونده")}</button>
              </div>
            </form>
          </div>
        </div>
